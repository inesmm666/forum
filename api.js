...
,
    {
		...
		console.log("fetch response status: ", response.status);
		if (!response.ok) {
			console.log("problem api forum 2");
		} else {
			console.log("tenho forum list");
			console.log("first "+response);
			console.log("json "+response.json());
			//sessionStorage.setItem("oi",JSON.stringify(response.json))
			return response.json();
...
//novo comentario
async function addCommentAPI(newActivity, idPost, onSucess, onError) {
	let username = getUsernameForFetch();
	let newActivityJson = JSON.stringify(newActivity);
	let authString = sessionStorage.getItem("Authorization");
	const rootPathC="http://localhost:8080/projeto-final-bagsimoes-irsilva-backend/rest/comments/";
	
	return await fetch(
		rootPathC+username + "add-comment/"+idPost,
		{
			method: "POST",
			headers: {
				Authorization: authString,
		...
		.then(function () {
			console.debug("tive sucesso ao criar message (api)"), onSucess();
		})
		.catch(function (error) {
			console.debug("houve um problema ao criar message (api): " + error);
			onError(error);
		});
}


//detalhes de comentario
async function apiGetDetalhesComentario(idAtividade) {
	let username = getUsernameForFetch();
	let authString = sessionStorage.getItem("Authorization");
	const rootPathForum ="http://localhost:8080/projeto-final-bagsimoes-irsilva-backend/rest/comments/comment-details/";
...
//responder comnetario

//ir buscar comentarios de post

//nova mensagem
async function addMessageAPI(newActivity, emailToSend, onSucess, onError) {
	let username = getUsernameForFetch();
	let newActivityJson = JSON.stringify(newActivity);
	let authString = sessionStorage.getItem("Authorization");
	const rootPathM="http://localhost:8080/projeto-final-bagsimoes-irsilva-backend/rest/message/";
	//http://localhost:8080/projeto-final-bagsimoes-irsilva-backend/rest/message/inesmm1996@gmail.com/sendMessage
	return await fetch(
		rootPathM+username + "/sendMessage",
		{
			method: "POST",
			headers: {
				Authorization: authString,
...
//http://localhost:8080/projeto-final-bagsimoes-irsilva-backend/rest/message/message-details/
async function apiGetDetalhesMessage(idAtividade) {
	let username = getUsernameForFetch();
	let authString = sessionStorage.getItem("Authorization");
	const rootPathForum ="http://localhost:8080/projeto-final-bagsimoes-irsilva-backend/rest/message/message-details/";
...
//apagar/ler mensagem
//http://localhost:8080/projeto-final-bagsimoes-irsilva-backend/rest/message/inesmm1996@gmail.com/updateMessage/1
async function updateMessageAPI(projecto, projectId,parentli, onSucess,onError) {
	let username = getUsernameForFetch();
	let authString = sessionStorage.getItem("Authorization");
	let projectJson = JSON.stringify(projecto);
	// let authString = JSON.stringify("123");
const caminho="http://localhost:8080/projeto-final-bagsimoes-irsilva-backend/rest/message/";

return await fetch(caminho + username + "/updateMessage/" + projectId, {
		method: "POST",
		headers: {
			Authorization: authString,
			"Content-Type": "application/json",
		...
		.then(function () {
			console.debug("tive sucesso a atualizar msg"),
			onSucess(ativClicada,parentli);
		})
		.catch(function (error) {
			console.debug("houve um problema no update mensagem: " + error);
			onError(error);
		});
}
...
//apagar/ler notificação

//adicionar relação com outro post

//remover relação com outro post


//adicionar user ao perfil

//remover user ao perfil
